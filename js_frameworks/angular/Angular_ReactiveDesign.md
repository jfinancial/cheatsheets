...
- We also define our shared `LoadingService`. We will use a `BehaviorSubject` to define our custom `Observable` (i.e. `loading$`). We can them subscribe to the `Subject` and it can also emit values so we use the subject to define to when our `Observable` emits the values `true` or `false`. (We use `BehaviourSubject` as this type of subject - which takes an initial value - remembers the last value emitted by the subject so if you subscribe late then you still get the last value.) We don't expose our subject as this is private to the service. *The best way to expose custom observables is to use a private `BehaviourSubject`*
