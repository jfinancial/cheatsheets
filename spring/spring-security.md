...
public class CustomAuthenticationFilter(AuthenticationManager authManager) extends UsernamePasswordAuthenticationFilter {
  ...
  protected void successfulAuthentication(HttpServletRequest request, HttpServletResponse response, FilterChain chain, Authentication authResult) throws IOException, ServletException {
    ...
    Algorithm algorithm = Algorithm.HMAC256("secret".getBytes()); //secret should be a real secret and loaded
...
```
### Implementing A CustomAuthorizationFilter
```java
import com.auth0.jwt.JWT;
...
public class CustomAuthorizationFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain) throws ServletException, IOException {
        if(request.getServletPath().equals("/api/login")){
            filterChain.doFilter(request,response); //do nothing if it's login
        } else{
            String authorizationHeader = request.getHeader(AUTHORIZATION);
            if(authorizationHeader != null && authorizationHeader.startsWith("Bearer ")){
                try {
                    String token = authorizationHeader.substring("Bearer ".length()); //if has bearer we know we are good
                    Algorithm algorithm = Algorithm.HMAC256("secret".getBytes()); //secret should be a real secret and loaded
                    ...
                    error.put("error_nessage", e.getMessage());
                    response.setContentType(APPLICATION_JSON_VALUE);
